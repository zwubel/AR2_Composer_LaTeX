...
Das Ziel dieses Verfahren ist es eine \textit{Oriented Bounding Box}(OBB) von jedem erkannten Würfel Marker zu erstellen. Die OBB beinhalten Informationen über die Größe, den Schwerpunkt, die vier Eckpunkte sowie den Rotationswinkel im Verhältnis zum Bildkoordinatensystem. Alle genannten Informationen werden in einem \texttt{RotatedRect} gespeichert, welches eine Objektklasse der \textit{OpenCV} Bibliothek ist.
...
\end{figure}
\subsubsection{ArUco Marker Tracking}\label{sec:ArucoMarkerTracking}\todo[inline, color=yellow]{Vera}
...
\subsection{Verfolgung und Registrierung der Marker}\todo[inline, color = yellow]{Vera}
...
Im folgenden wird auf die implementierten Funktionen zur Verfolgung der Marker eingegangen. Diese werden in der Klasse \texttt{IdMapping.cpp} zur Verfügung gestellt und in der Methode \texttt{trackMarker} aufgerufen.
\begin{itemize}
...
\item \texttt{isTranslatedMarker} hat die Aufgabe Marker zu finden, die bewegt wurden. Hier wird die Zuordnung auch innerhalb eines Bewegungsradius mit Hilfe von $\|v_m\|$  abgeschätzt. Diese Arte der Verfolgung kann unter Umständen zu einer falschen Zuordnung führen. Doch der große Vorteil ist der gering Kostenaufwand und mit Betrachtung der Umstände sinkt die Wahrscheinlichkeit einer Fehlinterpretation auf ein vertretbares Risiko. Bei einem dieser Umstände handelt es sich um die Annahme das ein Nutzer nur ein bis zwei Marker zum gleichen Zeitpunkt bewegen wird während alle weiteren unbewegten Marker vorab als unverändert einstuft wurden und nicht weiter berücksichtigt werden.
Ein anderer Fakt ist, dass durch die Verwendung der \textit{ArUco} Marker zu jedem Zeitpunkt ein Marker wieder eindeutig zu geordnet werden kann. Dies ist unter Anderem der Fall wenn der Markercode nur langsam in der näheren Umgebung weiterer Marker bewegt wird. Nach erfolgreicher Verfolgung wird die ermittelte RID zurückgeben.
