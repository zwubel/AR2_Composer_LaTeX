...
\subsubsection{Server-seitige Netzwerkanbindung}\todo[inline, color=green]{Laura}
...
\subsubsection{Marker Handles}\label{sec:markerHandles}\todo[inline]{Paul}
...
\subsubsection{Ein- und Ausblenden des Kontextmenüs} \label{sec:KontextMenuSwitch}\todo[inline]{Lukas}
\subsubsection{Berechnung der Gebäudeeigenschaften} \label{sec:KontextMenuInhalt}\todo[inline]{Laura}
%Neben der Anwendungsspzfischen ID des jweiligen Gebäudes ist wie auf Abbildung BLA zu sehen ....
...
\subsection{uEye Ansteuerung}\todo[inline, color=yellow]{Vera}
...
\subsubsection{Schnittstelle der beiden Kalibrierung} \label{sec:CalibSteuerung}\todo[inline, backgroundcolor=yellow]{Laura}
Wie eingangs erwähnt, dient die \texttt{Calibration.cpp}-Klasse als Schnittstelle für die Kamerakalibrierung (vgl. Kapitel \ref{sec:camCalib}) und die Kalibrierung des Arbeitsbereichs (vgl. Kapitel \ref{sec:planeCalib}). \\

\lstinputlisting[title=\lstname, caption={\texttt{runPoseEstimation()}-Methode in \texttt{Calibration.cpp}}, label=lst:runPoseEstimation, language={C++}, linerange=17-23, firstnumber=17]{Quellcode/Calibration.cpp} 

In dem Quellcode-Auszug~\ref{lst:runPoseEstimation} wird die \texttt{runPoseEstimation()}-Methode gezeigt, welche dafür zuständig ist, die \texttt{generateCamMatAndDistMat()}-Methode (vgl. Quellcode-Auszug \ref{lst:generateCamMatAndDistMat}) aus der \texttt{PoseEstimation.cpp}-Klasse aufzurufen. Die daraus resultierenden Schritte werden in Kapitel \ref{sec:camCalib} erläutert. Dort wird ebenso auf die \texttt{distCoeffs}, also die Koeffizienten zur Behebung der Linsenverzeichnung, und die \texttt{camMatrix} -- dort $M_{intrinsisch}$ genannt -- eingegangen. Im vorliegenden Quellcode-Auszug werden diese von der \texttt{PoseEstimation.cpp}-Klasse an die \texttt{PlaneAndAffineCalibration.cpp}-Klasse weitergegeben.\\
Wie in Kapitel \ref{sec:planeCalib} erklärt werden mittels der Arbeitsbereichskalibrierung zwei doch recht unterschiedliche Ziele verfolgt. Zum einen wird wie in Quellcode-Auszug \ref{lst:getPlaneCalibData} gezeigt der tatsächliche Arbeitsbereich der Anwendung festgelegt und zum anderen wird die affine Transformation von Weltkoordinaten in Unitykoordinaten berechnet (vgl. Quellcode-Auszug \ref{lst:generateAffineAndPlaneCalib}).\\
Bei ersterem Quellcode-Auszug wird entweder ein neuer Arbeitsbereich mittels der \texttt{getPlaneCalibData()}-Methode ermittelt, oder mit dem Aufruf der \texttt{loadImagePlane()}-Methode ein gespeicherter Arbeitsbereich geladen (vgl. Kapitel \ref{sec:saveCalib}).

\lstinputlisting[title=\lstname, caption={\texttt{getPlaneCalibData()}-Methode in \texttt{Calibration.cpp}}, label=lst:getPlaneCalibData, language={C++}, linerange=51-55, firstnumber=51]{Quellcode/Calibration.cpp} 

Die affine Transformation wird auf Grundlage der korrespondierenden Punktepaare (vgl. Kapitel \ref{sec:Korrespondenz}) berechnet. Hierzu wird wie in Quellcode-Auszug \ref{lst:generateAffineAndPlaneCalib} gezeigt \textcolor{red}{Fertig schreiben wenn PLaneCalib fertig} 

\lstinputlisting[title=\lstname, caption={\texttt{generateAffineAndPlaneCalib()}-Methode in \texttt{Calibration.cpp}}, label=lst:generateAffineAndPlaneCalib, language={C++}, linerange=37-46, firstnumber=37]{Quellcode/Calibration.cpp} 
...
\item[Umsetzung in Quellcode:] Die für die Kamerakalibrierung relevanten Klassen sind zum einen die \texttt{PoseEstimation.cpp} und zum anderen die \texttt{calibWithChessboard.cpp}. An dieser Stelle werden nur die Kernstücke des Algorithmus erwähnt und erläutert.

\lstinputlisting[title=\lstname, caption={\texttt{generateCamMatAndDistMat()}-Methode in \texttt{PoseEstimation.cpp}}, label=lst:generateCamMatAndDistMat, language={C++}, linerange=24-34, firstnumber=24]{Quellcode/PoseEstimation.cpp} 

Die \texttt{generateCamMatAndDistMat()}-Methode aus der \texttt{PoseEstimation.cpp}-Klasse dient als Schnittstelle der Kamerakalibrierung. Hier werden neben der \texttt{cameraMatrix} ($M_{intrinsisch}$ aus Gleichung \ref{equ:intrinsic}) auch die extrinsischen Parameter in Form von \texttt{rvecs} (Matrix $R$ aus Gleichung \ref{equ:Rotation}) und \texttt{tvecs} (Vektor $t$ aus Gleichung \ref{equ:Translation}) gesammelt. Zudem werden die \texttt{disCoeffs} (Koeffizienten zur Behebung der Linsenverzeichnung) auf null gesetzt. 

Die wichtigsten zwei Methoden der \texttt{calibWithChessboard.cpp}-Klasse sind im Anhang in den Quellcode-Auszügen \ref{lst:runCalibration} und \ref{lst:runCalibWithChessboard} dargestellt. 

\lstinputlisting[title=\lstname, caption={Auszug aus der \texttt{runCalibration()}-Methode in \texttt{calibWithChessboard.cpp}}, label=lst:runCalibration, language={C++}, linerange=90-91, firstnumber=90]{Quellcode/calibWithChessboard.cpp} 

In dem Quellcode-Auszug \ref{lst:runCalibration} wird der Aufruf der \textit{OpenCV}-Methode \texttt{calibrateCamera()} gezeigt, die sowohl die intrinsischen, als auch die extrinsischen Kameraparameter auf Grundlage mehrerer Aufnahmen eines Kalibrierungsmusters, in diesem Fall dem Schachbrett aus Abbildung \ref{fig:schachbrettKalib}, bestimmt.  
\end{description}
...
\begin{description}
\item[Begrenzung des Arbeitsbereichs] Die ersten 4 Punktepaare dienen dazu den Arbeitsbereich zu definieren. Sie müssen in einer klardefinierten Reihenfolge, nämlich im Uhrzeigersinn und in der linken unteren Ecke beginnend. Dabei wird bei beiden Systemen (Tracking und Unity) unterschiedlich vorgegangen, was zu leicht abweichenden Arbeitsbereichen führen kann. Der Unterschied ist allerdings vernachlässigbar, wenn der Benutzer sich an den Tischkanten orientiert und beispielsweise die Kante der Auflage des Kalibrierungscontrollers (vgl. Kapitel \ref{sec:calibController} daran ausrichtet und somit eine ungefähr rechteck-förmige Fläche aufspannt. \\
Für die Tracking-Anwendung reicht es aus, den 1. und 3. Punkt zu nehmen und damit ein Rechteck aufzuspannen. Dies geschieht in der \texttt{computePlaneCalibration()}-Methode in der \texttt{PlaneAndAffineCalibration.cpp}-Klasse. Der so definierte Arbeitsbereich dient hier vor allem dazu, zu entscheiden, ob ein Marker noch im Anwendungsbereich ist und somit fortlaufend getrackt werden muss, oder ob er diesen Bereich verlassen hat und seine ID, sowie alle anderen Eigenschaften zurückgesetzt werden müssen (vgl. Kapitel \ref{sec:ID}).\\
Auf Unity-Seite werden alle 4 Punkte genutzt um den Arbeitsbereich festzulegen. \textcolor{red}{Lukas bitte helfen :)}
...
\subsection{Marker Detektion}\todo[inline, color = yellow]{Vera}
...
\subsubsection{Detektion der grünen Rechtecke}\todo[inline, color=yellow]{Vera}
...
std::vector< cv::Point3d > tvecs; \ref{sec:planeCalib}
cv::Point3d estimatedCenter; \ref{sec:planeCalib} 

\subsection{Verfolgung der Identitäten} \label{sec:ID}\todo[inline]{Vera}
