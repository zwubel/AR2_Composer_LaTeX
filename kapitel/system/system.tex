...
\subsubsection{Ansteuerung der beiden Kalibrierungsarten} \label{sec:CalibSteuerung}\todo[inline, backgroundcolor=yellow]{Laura}
...
Die affine Transformation wird auf Grundlage der korrespondierenden Punktepaare (vgl. Kapitel \ref{sec:Korrespondenz}) berechnet. Hierzu wird wie in Quellcode-Auszug \ref{lst:generateAffineAndPlaneCalib} gezeigt \textcolor{red}{...} 
...
Die \texttt{generateCamMatAndDistMat()}-Methode aus der \texttt{PoseEstimation.cpp}-Klasse dient als Schnittstelle der Kamerakalibrierung. Hier werden neben der \texttt{cameraMatrix} ($M_{intrinsisch}$) auch die extrinsischen Parameter in Form von \texttt{rvecs} (Matrix $R$) und \texttt{tvecs} (Vektor $t$) gesammelt. Zudem werden die \texttt{disCoeffs} (Koeffizienten zur Behebung der Linsenverzeichnung) auf null gesetzt. 
...
Die wichtigsten Methoden der letzteren Klasse sind im Anhang in den Quellcode-Auszügen \ref{lst:runCalibration} und \ref{lst:runCalibWithChessboard} dargestellt. 

\lstinputlisting[title=\lstname, caption={Auszug aus der \texttt{runCalibration()}-Methode in \texttt{calibWithChessboard.cpp}}, label=lst:runCalibration, language={C++}, linerange=85-86, firstnumber=85]{Quellcode/calibWithChessboard.cpp} 
