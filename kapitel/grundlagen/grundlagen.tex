...
\subsection{Netzwerkverbindung anhand ISO/OSI-7-Schichtenmodell}\label{sec:Netzwerk}\todo[inline, color=yellow]{Laura}
...
Während die Schichten 1-4 als transportierende Schichten einzustufen sind, können die verbleibenden Schichten 5-7 als anwendungsorientiert angesehen werden. Da der Austausch von Daten für die Umsetzung von \textit{MaRC} im Vordergrund steht, wird im Folgenden besonders auf die vierte Schicht eingegangen. Dabei wird auf die beiden Übertragungsprotokolle \textit{Transmission Control Protocol} (TCP) und \textit{User Datagram Protocol}(UDP) eingegangen.
